 ...
 * | 2026-08-24 | `reject-session-gzip-data-uri` → `session-gzip-data-uri`, `throws` became `decodes` — one fixture moved, renamed with it | **#518**, ADR-0011 decision 2. The one row of #518's divergence table that closed towards acceptance: `data:application/gzip;base64,…` is a real data URI, which Spacewalk's decoder has always read under the same `?session=` parameter name and this one refused, because `data:` was tested as a five-character prefix and the remainder handed to `BGZip.uncompressString`. `decodeSessionString` now routes the `/gzip;base64` body to `BGZip.decodeDataURI` ahead of the raw-payload path, so the fixture records the session it always carried instead of the inflate failure. **This is a change to the wire format — the accepted set grew by one spelling**, which is why it is a deliberate deviation from the baseline and why ADR-0011 exists to be the comment ADR-0006 decision 1 requires for one. Nothing that already decoded moved, and nothing that is still refused stopped being refused: the other rows of the table are stated boundaries, not divergences to close. Every session fixture also gained a `payload` field, which the golden file does not read — it snapshots decoded output, not fixture shape — so no snapshot moved for it. |
 *
